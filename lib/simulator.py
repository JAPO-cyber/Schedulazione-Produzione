...
Ora con validazione colonne e gestione esplicita degli errori KeyError.
"""
import random
import simpy
import pandas as pd
...
def esegui_simulazione(
    ...
    # --------------------------------------------------
    # df_tempi
    required_tempi = {'Fase', 'Macchina'}  # colonne sempre richieste
    # la colonna tempo può chiamarsi "Tempo_Minuti" o "Tempo"
    if 'Tempo_Minuti' in df_tempi.columns:
        required_tempi.add('Tempo_Minuti')
    else:
        required_tempi.add('Tempo')
    # colonne ausiliarie
    for col in ('Pezzi', 'Addetti', 'EnergiaFase', 'Carrelli'):
    ...
    # ---------------------
    # 3) Data di start/fine simulazione
    # ---------------------
    ...
    def process_lotto(rec):
        ...
        for _, fr in df_tempi.iterrows():
            ...
            dur = t0 * eq + post_map.get((rec['ID_Lotto'], fase), 0) + fisio_map.get((rec['Formato'], fase, 'INIZIO_FASE'), 0)
            remaining = int(dur)
            while remaining > 0:
                now = env.now
    ...
    # ---------------------
    # 10) Avvio processi e gestione errori
    # ---------------------
    try:
        ...
        # Rilancio con messaggio esplicito
        raise KeyError(f"Simulazione interrotta: colonna mancante {e}")
